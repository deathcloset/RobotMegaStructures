import { encodeMessage } from '@rms/shared';
...
export class SimLoop {
  ...
  private broadcast(now: number): void {
    ...
    const entities = this.chunks.primary.fullSnapshot();
    let totalBytes = 0;
    let recipients = 0;
    ...
    this.metrics.recordBroadcast(totalBytes, recipients, entities.length);
    this.metrics.recordBroadcastTime(Date.now() - bStart);

    // periodic durable snapshot seam (no-op in memory; real in Phase 2/3)
    this.repo.saveChunkSnapshot(this.chunks.primary);
  }
}
